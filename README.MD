...
- **Drag and drop** icons between pages and the dock, stack two icons to make a folder, drag to
  Remove to get rid of one. The cell you are about to drop into is marked while you drag, and
  letting go where you picked up leaves the icon exactly where it was.
- **Folders open both ways.** Hold an app inside one to rename it, take it back out onto the page,
  or drag it anywhere else. Tap beside an open folder to close it, or use back - whichever you
  reach for, and you can turn the tap off in settings.
...
- **Icon packs**, any you have installed, read the way every other launcher reads them. Packs that
  ship a background and mask theme the apps they have no entry for too. The home screen appears at
  the speed it would with no pack at all and the pack lands on top of it, rather than making you
  wait on a blank screen while a few hundred icons are composed.
...
- **Widgets, hosted properly.** Every widget your phone provides, grouped by app, each one labelled
  with how many cells it will take *on your grid* rather than on some assumed one - and anything
  that cannot fit is shown but not offered, instead of failing after you have already agreed to it.
  Drag them between pages, and drag the handles to resize in whole cells.
- **A simple mode, if you want less.** One switch turns Eden back into what it was at v0.2.0 - a
  home screen, a drawer, wallpapers, and the settings for them. It puts things away; it never
  deletes them, and everything comes back the moment you switch it off.
- **A home screen that stays where you put it.** No swinging sideways because you set the phone down
  at an angle. Apps you open still rotate exactly as they did, and there is a setting if you
  disagree.
- **Nothing is destroyed on one tap.** Deleting a page says how many things are on it and waits for
  an answer; an empty page still goes without ceremony.

Coming next: icon shapes, then notification dots.
...
| **2** | Core launcher - loader, drag & drop, app drawer, folders, page management | **Done** |
...
| **5** | Widgets - `AppWidgetHost`, the bind flow, the picker, resize frames - plus simple mode | **Done** |
| 6 | Icon shapes, notification dots, page reorder by dragging cards | Next |

Running on a Galaxy M31 (Android 16, API 36). Release APK is about 830 KB, with no native code.
...
`-- app/src/main/
    ...
    `-- kotlin/app/auriel/edenlauncher/
        ...
        |-- widget/                 AppWidgetHost, the picker, resize frames
